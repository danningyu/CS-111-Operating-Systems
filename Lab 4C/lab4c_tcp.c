...
void shutdownHandler(){
    ...
    if(write(STDOUT_FILENO, shutdownBuffer, amtWritten)<0){
        fprintf(stderr, "lab 4C: Failed to write to stdout: %s\n", strerror(errno));
        exit(SYS_CALL_ERROR);
    }
    ...
    if(logFilePath != NULL){
        ...
        if(write(logFileFD, logFileBuffer, amtWritten)<0){
            fprintf(stderr, "lab 4C: Failed to write to log: %s\n", strerror(errno));
            exit(SYS_CALL_ERROR);
        }        
    }
...
void processArgument(char* argument){
	// argument is a null-byte terminated string (C string)
    if(DEBUG){
...
int main(int argc, char** argv){
    ...
    if(logFilePath == NULL || clientID == NULL || hostName == NULL){
        fprintf(stderr, "lab 4C usage: port, --log, --id, and --host parameters are mandatory: usage: %s\n", usageMessage);
        exit(ARGS_ERROR);
    }
    if(samplingFreq <= 0){
        fprintf(stderr, "lab 4C: sampling frequency must be at least 1\n");
    ...
    if(write(STDOUT_FILENO, intialMessage, amtWritten)<0){
            fprintf(stderr, "lab 4C: Failed to write to log: %s\n", strerror(errno));
            exit(SYS_CALL_ERROR);
    }
    ...
    if(tempSensor == NULL){
        fprintf(stderr, "lab 4C: Failed to initialize temperature sensor (AIO device)\n");
        mraa_deinit();
        exit(OTHER_ERROR);
    }
    if(DEBUG){
    ...
    if(gettimeofday(&currentTv, NULL) < 0){
        fprintf(stderr, "lab 4C: Failed to get current time: %s\n", strerror(errno));
        exit(SYS_CALL_ERROR);
    }
    ...
    while(1){
        ...
        if(shouldRead && currentTv.tv_sec >= readyToReadTv.tv_sec){
            ...
            if(tempSensorReading<0){
            	fprintf(stderr, "lab 4C: Error reading the temperature\n");
            	exit(OTHER_ERROR);
            }
            ...
            if(write(STDOUT_FILENO, outputBuffer, amtWritten)<0){
                fprintf(stderr, "lab 4C: Failed to write to stdout: %s\n", strerror(errno));
                exit(SYS_CALL_ERROR);
            }
            if(logFilePath != NULL){
        ...
        if(pollEvents[0].revents & POLLIN){
            ...
            if(fdInAmount < 0){
                fprintf(stderr, "lab 4C: error reading in text: %s\n", strerror(errno));
                exit(SYS_CALL_ERROR);
            }
